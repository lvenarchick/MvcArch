...
<%
	//Logger.getLogger("oasweb").debug("Enter csRequestConfirm.jsp");
...
response.addHeader("X-Frame-Options", "DENY");
	...
	String title= (String) request.getAttribute("confirmTitle"); 
	if (title == null){
		title = "";
	}
	...
	String confirmMsgTitle= (String) request.getAttribute("confirmMsgTitle"); 
	if (confirmMsgTitle == null){
		confirmMsgTitle = "";
	} 
	else {
		if (confirmMsgTitle.equals("Error")){
			confirmMsgTitle="<img src=\"images/alert.gif\" width=\"25\" height=\"25\">";
		}
	}
	//Logger.getLogger("oasweb").debug("title:"+title+",confirmMsgTitle:"+confirmMsgTitle+",confirmMsg:"+confirmMsg);
	...
	/* SRF 2008 0486 PPP Open Enrollment*/
   	...
   	if(pppEligiblityVO != null)
   	{
   		...
   		//Logger.getLogger("oas").debug("csRequestConfirm.jsp::["+pppEligibility+"],["+pppAccountNumber+"],["+pppExpDate+"]");
   	}
%>
<html>
...
function logoff()
{
	document.getElementById("csRequestConfirmForm").action="logoff.do";
	document.getElementById("csRequestConfirmForm").submit();
}

function accountSummary()
{
	document.getElementById("csRequestConfirmForm").action="accountSummary.do";
	document.getElementById("csRequestConfirmForm").submit();
}

function submitPPPEnrollment()
{
	document.getElementById("csRequestConfirmForm").action="pppEnrollmentInt.do";
	document.getElementById("csRequestConfirmForm").submit();
}

function doPayOnlineNow()
{
	document.getElementById("csRequestConfirmForm").action="oaspay.do";
	document.getElementById("csRequestConfirmForm").submit();
}

function doCustomerService()
{
	document.getElementById("csRequestConfirmForm").action="customerService.do";
	document.getElementById("csRequestConfirmForm").submit();
}
</script>
...
<center>
<form name="csRequestConfirmForm" id="csRequestConfirmForm" autocomplete="nope" method="post">
          ...
          <tr>
              ...
              <TBODY>
                <TR>
                  <TD width="37%" valign="top"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td width="60%" valign="top"><h1><%=title%><span class="data"><strong> </strong></span></h1></td>
                ...
                <tr>
                  <td valign="top">
				  <h2 align="center"><%=confirmMsgTitle%> </h2>
                    <!--<p>Your request has been submitted. A merchant dispute form will be mailed to the address on file. Please allow 7-10 business days to receive the form. Please complete and return as soon as possible to resolve your dispute. </p>
                    <p>&nbsp;</p>-->
					<p><%=confirmMsg%></p>
					<p>&nbsp;</p>
					</td>
           ...
           <%@ include file="links1.jsp" %>
        	...
        	<!-- Start Dynamic Content -->
           	<dynjsp:DynamicContent  page="CSREQCONF" pageLocation="a1"/>    
			<!-- End Dynamic Content -->
...
<%
	//Logger.getLogger("oasweb").debug("Exit csRequestConfirm.jsp");
%>